...
router.get('/deployment/:id', async (req, res) => {
    try {
        ...
        const yamls = {};
        yamls.service = await k8sUtils.runCommandRaw(`kubectl get svc -n ${global.config.namespace} ${req.params.id} -o yaml`);
        yamls.deployment = await k8sUtils.runCommandRaw(`kubectl get deploy -n ${global.config.namespace} ${req.params.id} -o yaml`);
